...
class ResNet(nn.Module):
    ...
    def init_params(self, layers=None, Norm=True, seed=0):
        ...
        for m in modules:
            ...
            if isinstance(m, nn.Conv2d):
                torch.manual_seed(seed)
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            elif Norm and isinstance(m, (nn.BatchNorm2d, nn.GroupNorm)):
                nn.init.constant_(m.weight, 1)
                nn.init.constant_(m.bias, 0)
    ...
    def reset_first_layer(self, num_channels, seed=0):
        ...
        self.init_params(layers=self.conv1, Norm=False, seed=seed)
...
def _resnet(arch, block, layers, pretrained, progress, num_channels, num_classes, **kwargs):
    if pretrained:
        model = ResNet(block, layers, num_channels=3, num_classes=1000, **kwargs)
        state_dict = load_state_dict_from_url(model_urls[arch], progress=progress)
        model.load_state_dict(state_dict)
        model.reset_first_layer(num_channels)
        model.reset_head(num_classes=num_classes)
    else:
        model = ResNet(block, layers, num_channels=num_channels, num_classes=num_classes, **kwargs)
    return model


def ResNet18(num_channels, num_classes):
    return _resnet('resnet18', BasicBlock, [2, 2, 2, 2], False, True, num_channels=num_channels, num_classes=num_classes)

def ResNet152(num_channels, num_classes):
    return _resnet('resnet152', Bottleneck, [3, 8, 36, 3], False, True, num_channels=num_channels, num_classes=num_classes)


if __name__ == '__main__':
    model = ResNet(num_channels=200, num_classes=3, block=BasicBlock, layers=[2, 2, 2, 2])
    model.reset()
