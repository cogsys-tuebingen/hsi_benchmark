...
class PermutatorBlock(nn.Module):
    def __init__(self, dim, segment_dim, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, skip_lam=1.0, mlp_fn=ConvPermuteMLP):
        super().__init__()
    ...
    def forward(self, x):
        dim = self.dim 
        self.norm1 = self.norm_layer(dim).to(x.device)
        self.norm2 = self.norm_layer(dim).to(x.device)
...
class HiT(nn.Module):
    ...
    based on the implementation of the paper  ""
    https://github.com/xiachangxue/DeepHyperX
    """

    def __init__(self, num_channels, num_classes, layers=[4, 3, 14, 3],
                 img_size=63, patch_size=3, embed_dims=[480, 512, 512], 
                 transitions=[False, True, False, False], segment_dim=[8, 8, 4, 4], 
                 mlp_ratios=[3, 3, 3, 3], skip_lam=1.0, qkv_bias=False, qk_scale=None, 
                 drop_rate=0.1, attn_drop_rate=0.1, drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, mlp_fn=ConvPermuteMLP):

        super().__init__()
        first_layer_dim_mapping = {
            103: 208,
            200: 400,
            204: 408,
            224: 448,
        ...
        first_layer_dim = first_layer_dim_mapping[num_channels] if num_channels in first_layer_dim_mapping else 200
        embed_dims = [first_layer_dim] + embed_dims

        self.num_classes = num_classes

        self.patch_embed = PatchEmbed(img_size=img_size, patch_size=patch_size, in_chans=num_channels,
