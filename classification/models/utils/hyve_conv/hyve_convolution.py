...
class HyVEConv(nn.Module):
    def __init__(self, num_of_wrois, wavelength_range,
        ...
        Args:
            num_of_wrois: This is parameter G, which defines the amount of used camera filters
            wavelength_range: A tuple, with: (min_wavelength, max_wavelength)
            enable_extension: Enables HvVEConv++ (default: True)
            stop_gaussian_gradient: Prevent Gaussian training. Only for test purpose (default: False)
    ...
    def initialize_gaussian_distribution(self, wavelength_range:Optional[Tuple[float, float]]=None, seed=0):
        if wavelength_range is not None:
            ...
            self.gauss = GaussDistributionModule(self.gauss_num,
                                                 ...
                                                 (self.wavelength_range[1] -
                                                  self.wavelength_range[0]),
                                                 self.gauss_variance_factor,
                                                 equally_distributed=self.gauss.equally_distributed)

        self.gauss.initialize_weights(seed=0)
        self.gauss.to(self.kernel_weights_individual.device)

    def initialize(self, seed=0):
        self.initialize_gaussian_distribution(seed=0)
